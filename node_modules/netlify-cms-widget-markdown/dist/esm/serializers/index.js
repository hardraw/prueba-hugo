...
export const markdownToRemark = markdown => {
   ...
   */
  const parsed = unified().use(markdownToRemarkPlugin, {
    fences: true,
    commonmark: true
  }).use(markdownToRemarkRemoveTokenizers, {
    inlineTokenizers: ['url']
  }).use(remarkAllowHtmlEntities).parse(markdown);
  /**
   * Further transform the MDAST with plugins.
   */

  const result = unified().use(remarkSquashReferences).use(remarkImagesToText).use(remarkShortcodes, {
...
export const remarkToMarkdown = obj => {
  ...
  const processedMdast = unified().use(remarkEscapeMarkdownEntities).use(remarkStripTrailingBreaks).runSync(mdast);
  const markdown = unified().use(remarkToMarkdownPlugin, remarkToMarkdownPluginOpts).use(remarkAllowAllText).stringify(processedMdast);
  /**
...
export const htmlToSlate = html => {
  ...
  const slateRaw = unified().use(remarkAssertParents).use(remarkPaddedLinks).use(remarkImagesToText).use(remarkShortcodes, {
    plugins: getEditorComponents()
  }).use(remarkWrapHtml).use(remarkToSlate).runSync(mdast);
  return slateRaw;
};
/**
...
export const markdownToSlate = markdown => {
  const mdast = markdownToRemark(markdown);
  const slateRaw = unified().use(remarkWrapHtml).use(remarkToSlate).runSync(mdast);
  return slateRaw;
};
/**
