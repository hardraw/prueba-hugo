...
function transform(node) {
  ...
  const children = !_isEmpty(combinedChildren) && _flatMap(combinedChildren, transform);
  /**
...
function combineTextAndInline(nodes) {
  return nodes.reduce((acc, node) => {
    const prevNode = _last(acc);

    const prevNodeLeaves = _get(prevNode, 'leaves');
    ...
    if (!_isEmpty(prevNodeLeaves) && !_isEmpty(data.marks)) {
      prevNodeLeaves.push({
        node,
        marks: data.marks
      });
    ...
    if (!_isEmpty(prevNodeLeaves) && !_isEmpty(node.leaves)) {
...
function processCodeMark(markTypes) {
  ...
  const filteredMarkTypes = isInlineCode ? _without(markTypes, 'inlineCode') : markTypes;
...
function condenseNodesReducer(acc, node, idx, nodes) {
  ...
  if (node.marks && node.marks.length > 0) {
    ...
    const parentMarkLength = _last(_sortBy(markLengths, 'length'));
    ...
    const denestedChildren = children.map(child => _objectSpread({}, child, {
      marks: _without(child.marks, parentType)
    }));
  ...
  const baseNode = typeof node.text === 'string' ? u(node.textNodeType, {
    marks: node.marks
...
function convertNode(node, children, shortcodePlugins) {
  switch (node.type) {
    ...
    case 'code':
      {
        const value = _flatMap(node.nodes, child => {
          return _flatMap(child.leaves, 'text');
        }).join('');
