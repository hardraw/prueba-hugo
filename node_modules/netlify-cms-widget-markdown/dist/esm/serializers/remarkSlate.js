...
function transform(node) {
   ...
   */
  const children = !['strong', 'emphasis', 'delete'].includes(node.type) && !_isEmpty(node.children) && _flatMap(node.children, transform).filter(val => val);
  /**
...
function createText(value, data) {
  ...
  const leaves = _isArray(value) ? value : [{
    text: value
  }];
...
function processMarkNode(node) {
  ...
  const children = _flatMap(node.children, childNode => {
...
function convertMarkNode(node) {
  ...
  const convertedSlateNodes = slateNodes.reduce((acc, node) => {
    const lastConvertedNode = _last(acc);
...
function convertNode(node, nodes) {
   ...
   * unexpected errors.
   */
  const type = _get(node, ['data', 'shortcode']) ? 'shortcode' : node.type;
