...
export default class API {
  constructor(config) {
    ...
    _defineProperty(this, "withAuthorizationHeaders", req => unsentRequest.withHeaders(this.token ? {
      Authorization: `Bearer ${this.token}`
    } : {}, req));

    _defineProperty(this, "buildRequest", req => _flow([unsentRequest.withRoot(this.api_root), this.withAuthorizationHeaders, unsentRequest.withTimestamp])(req));

    _defineProperty(this, "request", async req => _flow([this.buildRequest, unsentRequest.performRequest, p => p.catch(err => Promise.reject(new APIError(err.message, null, 'GitLab')))])(req));
    ...
    _defineProperty(this, "responseFormats", fromJS({
      json: async res => {
    ...
    _defineProperty(this, "getCursorFromHeaders", headers => {
      ...
      const links = List(linksRaw.split(',')).map(str => str.trim().split(';')).map((_ref6) => {
        let _ref7 = _slicedToArray(_ref6, 2),
            linkStr = _ref7[0],
            keyStr = _ref7[1];

        return [keyStr.match(/rel="(.*?)"/)[1], unsentRequest.fromURL(linkStr.trim().match(/<(.*?)>/)[1].replace(/\+/g, '%20'))];
    ...
    _defineProperty(this, "fetchCursorAndEntries", req => _flow([unsentRequest.withMethod('GET'), this.request, p => Promise.all([p.then(this.getCursor), p.then(this.responseToJSON)]), then((_ref9) => {
    ...
    _defineProperty(this, "reversableActions", Map({
      first: 'last',
      last: 'first',
      next: 'prev',
      prev: 'next'
    ...
    _defineProperty(this, "uploadAndCommit", async (item, _ref19) => {
      ...
      const content = await _result(item, 'toBase64', _partial(this.toBase64, item.raw));
    ...
    _defineProperty(this, "deleteFile", function (path, commit_message) {
      ...
      return _flow([unsentRequest.withMethod('DELETE'), // TODO: only send author params if they are defined.
      unsentRequest.withParams(commitParams), _this.request])(`${_this.repoURL}/repository/files/${encodeURIComponent(path)}`);
    });
