...
export const createPointerFile = (_ref2) => {
  let size = _ref2.size,
      sha = _ref2.sha;
  return `\
...
const parseGitAttributesPatternLine = _flow([splitIntoWords, (_ref3) => {
...
export const getLargeMediaPatternsFromGitAttributesFile = _flow([parseGitAttributesFileToPatternAttributePairs, _filter( // eslint-disable-next-line no-unused-vars
(_ref5) => {
...
export const matchPath = (_ref9, path) => {
...
const getResourceDownloadURLs = (clientConfig, objects) => getResourceDownloadURLArgs(clientConfig, objects).then(_map(downloadURLArg => getDownloadURL(downloadURLArg))).then(Promise.all.bind(Promise));
...
export const getClient = clientConfig => {
  return _flow([Object.keys, _map(key => [key, configureFn(clientConfig, clientFns[key])]), _fromPairs, configuredFns => _objectSpread({}, configuredFns, {
