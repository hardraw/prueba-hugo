...
export default class GitGateway {
  constructor(config) {
    ...
    _defineProperty(this, "requestFunction", req => this.tokenPromise().then(token => unsentRequest.withHeaders({
    ...
    this.authClient = window.netlifyIdentity ? window.netlifyIdentity.gotrue : new GoTrue({
      APIUrl
    });
  ...
  authenticate(user) {
    ...
    return this.tokenPromise().then(async token => {
      ...
      if (this.acceptRoles && this.acceptRoles.length > 0) {
        const userRoles = _get(jwtDecode(token), 'app_metadata.roles', []);

        const validRole = _intersection(userRoles, this.acceptRoles).length > 0;
      ...
      const apiConfig = {
        ...
        commitAuthor: _pick(userData, ['name', 'email']),
      ...
      if (this.backendType === 'github') {
        ...
        this.backend = new GitHubBackend(this.config, _objectSpread({}, this.options, {
          API: this.api
        }));
      } else if (this.backendType === 'gitlab') {
        this.api = new GitLabAPI(apiConfig);
        this.backend = new GitLabBackend(this.config, _objectSpread({}, this.options, {
          API: this.api
        }));
      } else if (this.backendType === 'bitbucket') {
        this.api = new BitBucketAPI(_objectSpread({}, apiConfig, {
          requestFunction: this.requestFunction,
          hasWriteAccess: async () => true
        }));
        this.backend = new BitbucketBackend(this.config, _objectSpread({}, this.options, {
          API: this.api
        }));
      }
  ...
  _getLargeMediaClient() {
    const netlifyLargeMediaEnabledPromise = this.api.readFile('.lfsconfig').then(ini.decode).then((_ref5) => {
    ...
    const lfsPatternsPromise = this.api.readFile('.gitattributes').then(getLargeMediaPatternsFromGitAttributesFile).then(patterns => ({
      patterns
  ...
  async getLargeMediaDisplayURLs(mediaFiles) {
    ...
    })).then(_unzip).then((_ref11) => {
      let _ref12 = _slicedToArray(_ref11, 2),
          idMaps = _ref12[0],
          files = _ref12[1];

      return Promise.all([idMaps, client.getResourceDownloadURLArgs(files).then(_fromPairs)]);
    }).then((_ref13) => {
