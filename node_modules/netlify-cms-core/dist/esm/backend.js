...
import { selectListMethod, selectEntrySlug, selectEntryPath, selectAllowNewEntries, selectAllowDeletion, selectFolderEntryExtension, selectIdentifier, selectInferedField } from "./reducers/collections";
...
function slugFormatter(collection, entryData, slugConfig) {
  ...
  const processSlug = _flow([compileSlug, prepareSlug, _partialRight(sanitizeSlug, slugConfig)]);
...
function createPreviewUrl(baseUrl, collection, slug, slugConfig, entry) {
  ...
  const processSegment = _flow([value => String(value), prepareSlug, _partialRight(sanitizeSlug, slugConfig)]);
...
class Backend {
  ...
  async search(collections, searchTerm) {
    ...
    const collectionEntriesRequests = collections.map(async collection => {
      ...
      const searchFields = [selectInferedField(collection, 'title'), selectInferedField(collection, 'shortTitle'), selectInferedField(collection, 'author')];
      const collectionEntries = await this.listAllEntries(collection);
      return fuzzy.filter(searchTerm, collectionEntries, {
        extract: extractSearchFields(searchFields)
      });
    }).map(p => p.catch(err => errors.push(err) && []));
    const entries = await Promise.all(collectionEntriesRequests).then(arrs => _flatten(arrs));
  ...
  async getLocalDraftBackup(collection, slug) {
    ...
    return this.entryWithFormat(collection, slug)(createEntry(collection.get('name'), slug, path, {
      raw,
      label
    }));
  }
  ...
  getEntry(collection, slug) {
    ...
    const label = getLabelForFileCollectionEntry(collection, path);
    return this.implementation.getEntry(collection, slug, path).then(loadedEntry => this.entryWithFormat(collection, slug)(createEntry(collection.get('name'), slug, loadedEntry.file.path, {
      raw: loadedEntry.data,
      label
    })));
  }
