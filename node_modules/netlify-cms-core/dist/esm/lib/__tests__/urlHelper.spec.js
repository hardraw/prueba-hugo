...
describe('sanitizeURI', () => {
  ...
  it('should not keep valid non-latin chars (ucschars in RFC 3987) if set to ASCII mode', () => {
    expect(sanitizeURI('ěščřžý日本語のタイトル', {
      encoding: 'ascii'
    })).toEqual('');
  });
...
describe('sanitizeSlug', () => {
  ...
  it('should remove accents with `clean_accents` set', () => {
    expect(sanitizeSlug('ěščřžý', Map({
      clean_accents: true
    }))).toEqual('escrzy');
  });
  it('should remove non-latin chars in "ascii" mode', () => {
    expect(sanitizeSlug('ěščřžý日本語のタイトル', Map({
      encoding: 'ascii'
    }))).toEqual('');
  });
  ...
  it('uses alternate replacements', () => {
    expect(sanitizeSlug('test   test   ', Map({
