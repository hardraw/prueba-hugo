...
export function searchEntries(searchTerm) {
  ...
  return (dispatch, getState) => {
    ...
    const searchPromise = integration ? getIntegrationProvider(state.integrations, backend.getToken, integration).search(collections, searchTerm, page) : backend.search(state.collections.valueSeq().toArray(), searchTerm);
...
export function query(namespace, collectionName, searchFields, searchTerm) {
  return (dispatch, getState) => {
    ...
    const queryPromise = integration ? getIntegrationProvider(state.integrations, backend.getToken, integration).searchBy(searchFields.map(f => `data.${f}`), collectionName, searchTerm) : backend.query(collection, searchFields, searchTerm);
