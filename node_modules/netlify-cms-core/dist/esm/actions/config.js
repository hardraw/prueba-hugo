...
const getConfigUrl = () => {
  ...
  const isValidLink = configLinkEl && validTypes[configLinkEl.type] && _get(configLinkEl, 'href');

  if (isValidLink) {
    const link = _get(configLinkEl, 'href');
...
export function applyDefaults(config) {
  return Map(defaults).mergeDeep(config).withMutations(map => {
    ...
    const defaultPublicFolder = `/${_trimStart(map.get('media_folder'), '/')}`;
    ...
    map.set('collections', map.get('collections').map(collection => {
      ...
      if (folder) {
        return collection.set('folder', _trimStart(folder, '/'));
      }
      ...
      if (files) {
        return collection.set('files', files.map(file => {
          return file.set('file', _trimStart(file.get('file'), '/'));
        }));
      }
    }));
  });
