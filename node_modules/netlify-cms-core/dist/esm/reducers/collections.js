...
const selectors = {
  [FOLDER]: {
    entryExtension(collection) {
      return (collection.get('extension') || _get(formatExtensions, collection.get('format') || 'frontmatter')).replace(/^\./, '');
    },
    ...
    entrySlug(collection, path) {
      return path.split('/').pop().replace(new RegExp(`\\.${_escapeRegExp(this.entryExtension(collection))}$`), '');
    },
...
export const selectInferedField = (collection, fieldName) => {
  ...
  if (inferableField.showError) {
    consoleError(`The Field ${fieldName} is missing for the collection “${collection.get('name')}”`, `Netlify CMS tries to infer the entry ${fieldName} automatically, but one couldn't be found for entries of the collection “${collection.get('name')}”. Please check your site configuration.`);
  }
