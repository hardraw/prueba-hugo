import _styled from "@emotion/styled-base";

function _defineProperty(obj, key, value) { if (key in obj) { Object.defineProperty(obj, key, { value: value, enumerable: true, configurable: true, writable: true }); } else { obj[key] = value; } return obj; }

import React from 'react';
import PropTypes from 'prop-types';
import { translate } from 'react-polyglot';
import copyToClipboard from 'copy-text-to-clipboard';
import { localForage } from 'netlify-cms-lib-util';
import { buttons, colors } from 'netlify-cms-ui-default';
const ISSUE_URL = 'https://github.com/netlify/netlify-cms/issues/new?template=bug_report.md';

const ErrorBoundaryContainer = _styled("div", {
  target: "e177kfaq0",
  label: "ErrorBoundaryContainer"
})("padding:40px;h1{font-size:28px;color:", colors.text, ";}h2{font-size:20px;}strong{color:", colors.textLead, ";font-weight:500;}hr{width:200px;margin:30px 0;border:0;height:1px;background-color:", colors.text, ";}a{color:", colors.text, ";}" + (process.env.NODE_ENV === "production" ? "" : "/*# sourceMappingURL=data:application/json;charset=utf-8;base64, */"));

const CopyButton = _styled("button", {
  target: "e177kfaq1",
  label: "CopyButton"
})(buttons.button, ";", buttons.default, ";", buttons.gray, ";display:block;margin:12px 0;" + (process.env.NODE_ENV === "production" ? "" : "/*# sourceMappingURL=data:application/json;charset=utf-8;base64, */"));

const RecoveredEntry = (_ref) => {
  let entry = _ref.entry,
      t = _ref.t;
  console.log(entry);
  return React.createElement(React.Fragment, null, React.createElement("hr", null), React.createElement("h2", null, t('ui.errorBoundary.recoveredEntry.heading')), React.createElement("strong", null, t('ui.errorBoundary.recoveredEntry.warning')), React.createElement(CopyButton, {
    onClick: () => copyToClipboard(entry)
  }, t('ui.errorBoundary.recoveredEntry.copyButtonLabel')), React.createElement("pre", null, React.createElement("code", null, entry)));
};

class ErrorBoundary extends React.Component {
  constructor() {
    super(...arguments);

    _defineProperty(this, "state", {
      hasError: false,
      errorMessage: '',
      backup: ''
    });
  }

  static getDerivedStateFromError(error) {
    console.error(error);
    return {
      hasError: true,
      errorMessage: error.toString()
    };
  }

  shouldComponentUpdate(nextProps, nextState) {
    if (this.props.showBackup) {
      return this.state.errorMessage !== nextState.errorMessage || this.state.backup !== nextState.backup;
    }

    return true;
  }

  async componentDidUpdate() {
    if (this.props.showBackup) {
      const backup = await localForage.getItem('backup');
      console.log(backup);
      this.setState({
        backup
      });
    }
  }

  render() {
    const _this$state = this.state,
          hasError = _this$state.hasError,
          errorMessage = _this$state.errorMessage,
          backup = _this$state.backup;
    const _this$props = this.props,
          showBackup = _this$props.showBackup,
          t = _this$props.t;

    if (!hasError) {
      return this.props.children;
    }

    return React.createElement(ErrorBoundaryContainer, null, React.createElement("h1", null, t('ui.errorBoundary.title')), React.createElement("p", null, React.createElement("span", null, t('ui.errorBoundary.details')), React.createElement("a", {
      href: ISSUE_URL,
      target: "_blank",
      rel: "noopener noreferrer"
    }, t('ui.errorBoundary.reportIt'))), React.createElement("hr", null), React.createElement("h2", null, t('ui.errorBoundary.detailsHeading')), React.createElement("p", null, errorMessage), backup && showBackup && React.createElement(RecoveredEntry, {
      entry: backup,
      t: t
    }));
  }

}

_defineProperty(ErrorBoundary, "propTypes", {
  children: PropTypes.node,
  t: PropTypes.func.isRequired
});

export default translate()(ErrorBoundary);