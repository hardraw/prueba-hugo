...
import { localForage, filterPromises, resolvePromiseProperties, APIError, EditorialWorkflowError } from 'netlify-cms-lib-util';
...
export default class API {
  ...
  readFile(path, sha) {
    ...
    if (sha) {
      ...
      return this.request(`${this.repoURL}/contents/${path}`, {
        ...
        if (_hasIn(error, 'message.errors') && _find(error.message.errors, {
          code: 'too_large'
        })) {
  ...
  listUnpublishedBranches() {
    ...
    return this.request(`${this.repoURL}/git/refs/heads/cms`).then(branches => filterPromises(branches, branch => {
  ...
  editorialWorkflowGit(fileTree, entry, filesList, options) {
    ...
    if (!unpublished) {
      ...
      return this.getBranch(branchName).then(branchData => this.updateTree(branchData.commit.sha, '/', fileTree)).then(changeTree => this.commit(options.commitMessage, changeTree)).then(commit => {
        ...
        const metadataFiles = _get(metadata.objects, 'files', []);
  ...
  rebaseSingleBlobCommits(baseCommit, commits, pathToBlob) {
    ...
    if (commits.length === 0 || commits[0].parents[0].sha === baseCommit.sha) {
      return Promise.resolve(_last(commits));
    }
    /**
  ...
  assertHead(commits, headToAssert) {
    const headIsMissing = headToAssert.parents[0].sha === _last(commits).sha;

    const headIsNotMissing = headToAssert.sha === _last(commits).sha;
  ...
  getBlobInTree(treeSha, pathToBlob) {
    ...
    const subTreePromise = directories.reduce((treePromise, segment) => {
      return treePromise.then(tree => {
        const subTreeSha = _find(tree.tree, {
          path: segment
        }).sha;
    ...
    return subTreePromise.then(subTree => _find(subTree.tree, {
      path: filename
    }));
  }
  ...
  uploadBlob(item) {
    const content = _result(item, 'toBase64', _partial(this.toBase64, item.raw));
